...
const login = async (req, res) => {
    try {
        const { error, value } = (0, validator_1.loginValidator)(req.query);
        if (!error) {
...
const register = async (req, res) => {
    try {
        const { error, value } = (0, validator_1.registerValidator)(req.query);
        if (!error) {
...
const finduser = async (req, res) => {
    try {
        const { error, value } = (0, validator_1.finduserValidator)(req.query);
        if (!error) {
...
const followunfollow = async (req, res) => {
    try {
        const { error, value } = (0, validator_1.followunfollowValidator)(req.query);
        if (!error) {
